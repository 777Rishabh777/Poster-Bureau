    ...
    if (!initPromise) {
        initPromise = (async () => {
            ...
            if (mongoURI) {
                ...
                console.warn('[startup] MONGO_URI/MONGODB_URI not set. Using file storage.');
            }

            // Vercel functions use a read-only deployment filesystem, so persistent writes require MongoDB.
            if (isVercelRuntime && mode !== 'mongo') {
                if (!mongoURI) {
                    throw new Error('MONGO_URI or MONGODB_URI is required in Vercel deployments. Configure MongoDB Atlas and set one of these variables in Vercel environment variables.');
                }

                const reason = mongoConnectError && mongoConnectError.message
                    ? mongoConnectError.message
                    : 'Unknown MongoDB connection error';
                throw new Error(`MongoDB connection failed in Vercel: ${reason}. Verify credentials in URI and Atlas Network Access.`);
            }
